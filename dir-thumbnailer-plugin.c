...
#ifdef DEBUG
  g_message (_("Initializing the Tumbler Directory Thumbnailer plugin"));
#endif

  /* register the types provided by this plugin */
  dir_thumbnailer_register (plugin);
  dir_thumbnailer_provider_register (plugin);

  /* set up the plugin provider type list */
  type_list[0] = TYPE_DIR_THUMBNAILER_PROVIDER;
}
...
{
#ifdef DEBUG
  g_message (_("Shutting down the Tumbler Directory Thumbnailer plugin"));
#endif
}
